...
public class TTC {
    ...
    public static File TTCConverter(File directory, File inputFile, String outputName, boolean aCheck, boolean vCheck){
        ...
        try {
                ...
                while (sc.hasNextLine()){
                    ...
                    switch (currentList.get(6)) {
                        ...
                        default:
                            ...
                            objRotation =currentList.get(6);
                            lengthArray.get(row).add(objRotation);
                            break;
                    }

                    //Assigns StartX and startPos a value, then adds startPos to lengthArray
                    double StartX = 0.277778 * 3 * Double.parseDouble(currentList.get(1)) * Double.parseDouble(currentList.get(3));
                    String startPos = "\"" + (-StartX) + ",0,2.8288\"";
                    lengthArray.get(row).add(startPos);
                    //Assigns EndX and endPos a value, then adds endPos to lengthArray
                    double EndX = 0.277778 * Double.parseDouble(currentList.get(1)) * Double.parseDouble(currentList.get(3));
                    String endPos = "\"" + (-EndX) + ",0,2.8288\"";
                    ...
                    //Assigns StartX and startPos a value, then adds startPos to lengthArray
                    StartX = 0.277778 * 3 * Double.parseDouble(currentList.get(1)) * Double.parseDouble(currentList.get(3));
                    startPos = "\"" + (-StartX) + ",0,2.8288\"";
                    lengthArray.get(row).add(startPos);
                    //Assigns EndX and endPos a value, then adds endPos to lengthArray
                    EndX = 0.277778 * Double.parseDouble(currentList.get(1)) * Double.parseDouble(currentList.get(3));
                    endPos = "\"" + (-EndX) + ",0,2.8288\"";
                    lengthArray.get(row).add(endPos);
            ...
            //If there is the audio object type
                ...
                while (sc.hasNextLine()){
                    ...
                    lengthArray.get(row).add("\"0,0,0\"");
                    lengthArray.get(row).add("\"0,0,0\"");


                    //Assigns StartX and startPos a value, then adds startPos to lengthArray
                    double StartX = 0.277778 * 3 * Double.parseDouble(currentList.get(2)) * Double.parseDouble(currentList.get(4));
                    String startPos = "\"" + (-StartX) + ",0,2.8288\"";
                    lengthArray.get(row).add(startPos);
                    //Assigns EndX and endPos a value, then adds endPos to lengthArray
                    double EndX = 0.277778 * Double.parseDouble(currentList.get(2)) * Double.parseDouble(currentList.get(4));
                    String endPos = "\"" + (-EndX) + ",0,2.8288\"";
                    lengthArray.get(row).add(endPos);
                ...
                while (sc.hasNextLine()){
                    ...
                    lengthArray.get(row).add(objType);
                    //Adds gain to lengthArray
                    lengthArray.get(row).add(String.valueOf(gain));
                    //Assigns customMotion a value then adds it to lengthArray
                    String customMotion = "FALSE";
                    lengthArray.get(row).add(customMotion);
                    //Assigns customFile a value then adds it to lengthArray
